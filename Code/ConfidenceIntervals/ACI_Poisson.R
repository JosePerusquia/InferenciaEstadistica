#####################################################################
# Asymptotic confidence interval for Poisson(lambda) 
...
#####################################################################
# Create 100 confidence intervels with 10000 samples of Pois(5)
n=100
...
for(i in 1:100){
  L[i]=emv[i]-(qnorm(.975)*sqrt(emv[i]/n))
  U[i]=emv[i]+(qnorm(.975)*sqrt(emv[i]/n))
}
#####################################################################
...
res = res%>%
  mutate(flag=ifelse(L<5,ifelse(U>5,1,0),0))
